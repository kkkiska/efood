export const menuData = {
    Burger: [
        {
            id: 0,
            name: 'Cheeseburger With Salad',
            price: '18.00',
            rating: 5,
            img: '/images/burger1.png'
        },
        {
            id: 1,
            name: 'Beef Burger',
            price: '15.00',
            rating: 4.5,
            img: '/images/burger2.png'
        },
        {
            id: 2,
            name: 'Royel Cheeseburger',
            price: '16.00',
            rating: 4,
            img: '/images/burger3.png'
        },
        {
            id: 3,
            name: 'Black Gambugrer ',
            price: '14.00',
            rating: 5,
            img: '/images/burger4.png'
        },
        {
            id: 4,
            name: 'Chicken Burger',
            price: '15.00',
            rating: 5,
            img: '/images/burger5.png'
        },
        {
            id: 5,
            name: 'Tasty Burger',
            price: '20.00',
            rating: 4,
            img: '/images/burger6.png'
        },
    ],
    Pizza: [
        {
            name: 'Mushroom pizza',
            price: '18.00',
            rating: 5,
            img: '/images/pizza1.png'
        },
        {
            name: 'Greece pizza',
            price: '17.00',
            rating: 4.5,
            img: '/images/pizza2.png'
        },
        {
            name: 'Dou pizza',
            price: '19.00',
            rating: 4.5,
            img: '/images/pizza3.png'
        }
    ],
    Sandwich: [
        {
            name: 'Sandwich classic',
            price: '8.00',
            rating: 5,
            img: '/images/sandwich1.png'
        },
        {
            name: 'Sandwich belgium',
            price: '10.00',
            rating: 5,
            img: '/images/sandwich2.png'
        },
        {
            name: 'Sandwich mexican',
            price: '11.00',
            rating: 5,
            img: '/images/sandwich3.png'
        },
    ],
    "Asian Food": [
        {
            name: 'Lapsha',
            price: '21.00',
            rating: 5,
            img: '/images/asian1.png'
        },
        {
            name: 'Lapsha teriyaki',
            price: '25.00',
            rating: 5,
            img: '/images/asian2.png'
        },
        {
            name: 'Sushi',
            price: '16.00',
            rating: 5,
            img: '/images/asian3.png'
        },
    ],
    "Set Menu": [
        {
            name: 'Healthy set',
            price: '33.00',
            rating: 5,
            img: '/images/set1.png'
        },
        {
            name: 'Burger set',
            price: '30.00',
            rating: 5,
            img: '/images/set2.png'
        },
        {
            name: 'Mangal set',
            price: '45.00',
            rating: 5,
            img: '/images/set3.png'
        },
        {
            name: 'Beer set',
            price: '55.00',
            rating: 5,
            img: '/images/set4.png'
        },

    ]
} 